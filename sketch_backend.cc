  ...
  for (const auto * child_decl : dyn_cast<DeclContext>(tu_decl)->decls()) {
    ...
    if (isa<FunctionDecl>(child_decl) and
      ...
      if (ret != 0) {
        ...
        out << sketch_contents;
        out.close();
        std::cerr << "// Sketch failed to find a configuration";
        // with random seed " << rand_seed << " , input available at /tmp/debug.sk" << std::endl;
      } else {
        std::ofstream out("/tmp/success.sk");
        out << sketch_contents;
        out.close();
        std::cerr << "// Return value is " << ret << std::endl;
        std::cout << "Sketch succeeded";
        // with random seed " << rand_seed << " , input sketch available at /tmp/success.sk" << std::endl;
      }
    }
  }
