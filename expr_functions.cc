...
std::set<std::string> ExprFunctions::get_all_state_vars(const clang::Expr * expr) {
  // Get all stateful right hand sides of inst
  assert(expr);
  if (isa<ParenExpr>(expr)) {
    return get_all_state_vars(dyn_cast<ParenExpr>(expr)->getSubExpr());
  } else if (isa<CastExpr>(expr)) {
    return get_all_state_vars(dyn_cast<CastExpr>(expr)->getSubExpr());
  } else if (isa<UnaryOperator>(expr)) {
    return get_all_state_vars(dyn_cast<UnaryOperator>(expr)->getSubExpr());
    ...
    auto cond_set = get_all_state_vars(cond_op->getCond());
    auto true_set = get_all_state_vars(cond_op->getTrueExpr());
    auto false_set = get_all_state_vars(cond_op->getFalseExpr());
    ...
    auto lhs_set = get_all_state_vars(bin_op->getLHS());
    auto rhs_set = get_all_state_vars(bin_op->getRHS());
    std::set<std::string> ret;
...
std::string ExprFunctions::replace_state_vars(const clang::Expr * expr,
                                              const std::map<std::string, std::string> & repl_map) {
  // Get all stateful right hand sides of inst
  assert(expr);
  if (isa<ParenExpr>(expr)) {
    return replace_state_vars(dyn_cast<ParenExpr>(expr)->getSubExpr(), repl_map);
  } else if (isa<CastExpr>(expr)) {
    return replace_state_vars(dyn_cast<CastExpr>(expr)->getSubExpr(), repl_map);
  } else if (isa<UnaryOperator>(expr)) {
    return replace_state_vars(dyn_cast<UnaryOperator>(expr)->getSubExpr(), repl_map);
  } else if (isa<ConditionalOperator>(expr)) {
    const auto * cond_op = dyn_cast<ConditionalOperator>(expr);

    const auto cond_str = replace_state_vars(cond_op->getCond(), repl_map);
    const auto true_str = replace_state_vars(cond_op->getTrueExpr(), repl_map);
    const auto false_str = replace_state_vars(cond_op->getFalseExpr(), repl_map);

    return "(" + cond_str + ") ? (" + true_str + ") : (" + false_str + ")";
  } else if (isa<BinaryOperator>(expr)) {
    const auto * bin_op = dyn_cast<BinaryOperator>(expr);
    const auto lhs_str = replace_state_vars(bin_op->getLHS(), repl_map);
    const auto rhs_str = replace_state_vars(bin_op->getRHS(), repl_map);
    return lhs_str + std::string(BinaryOperator::getOpcodeStr(bin_op->getOpcode())) + rhs_str;
  } else if (isa<DeclRefExpr>(expr)) {
    // All DeclRefExpr are stateful variables
    return repl_map.at(clang_stmt_printer(expr));
  } else if (isa<MemberExpr>(expr)) {
    // All MemberExpr are packet variables
    ...
    std::string ret = "(";
    for (const auto * child : call_expr->arguments()) {
      const auto child_str = replace_state_vars(child, repl_map);
      ret += child_str + ",";
    }
    ret.back() = ')';
    return ret;
