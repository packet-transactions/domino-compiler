...
std::string
AstVisitor::ast_visit_transform(const TranslationUnitDecl *tu_decl) {
  return pkt_func_transform(
      tu_decl, std::bind(&AstVisitor::ast_visit_helper, this, _1, _2));
}

std::pair<std::string, std::vector<std::string>>
AstVisitor::ast_visit_helper(const clang::CompoundStmt *body,
                             const std::string &pkt_name
                             __attribute__((unused))) {
  return std::make_pair("{" + ast_visit_stmt(body) + "}",
...
std::string AstVisitor::ast_visit_comp_stmt(const CompoundStmt *comp_stmt) {
  ...
  for (const auto &child : comp_stmt->children()) {
    ...
    else if (ast_visit_stmt(child).find("if") != std::string::npos &&
             ast_visit_stmt(child).find('}') != std::string::npos) {
      ret += ast_visit_stmt(child);
    } else {
      ret += ast_visit_stmt(child) + ";";
    }
...
std::string
AstVisitor::ast_visit_decl_ref_expr(const DeclRefExpr *decl_ref_expr) {
  assert_exception(decl_ref_expr);
  return clang_stmt_printer(decl_ref_expr);
}

std::string AstVisitor::ast_visit_array_subscript_expr(
    const ArraySubscriptExpr *array_subscript_expr) {
  assert_exception(array_subscript_expr);
  return clang_stmt_printer(array_subscript_expr);
}

std::string
AstVisitor::ast_visit_integer_literal(const IntegerLiteral *integer_literal) {
...
std::string
AstVisitor::ast_visit_implicit_cast(const ImplicitCastExpr *implicit_cast) {
