...
std::string ChipmunkAnotherDominoGenerator::ast_visit_record_decl(
...
std::string
ChipmunkAnotherDominoGenerator::ast_visit_stmt_specially_for_if_condition(
    const Stmt *stmt) {
  ...
  if (isa<CompoundStmt>(stmt)) {
    ...
    if ((opcode == clang::BinaryOperatorKind::BO_LAnd or
         opcode == clang::BinaryOperatorKind::BO_Or) and
        rand == 3)
      return ast_visit_stmt_specially_for_if_condition(bin_op->getRHS()) +
             std::string(bin_op->getOpcodeStr()) +
             ast_visit_stmt_specially_for_if_condition(bin_op->getLHS());
    else
      return ast_visit_stmt_specially_for_if_condition(bin_op->getLHS()) +
             std::string(bin_op->getOpcodeStr()) +
             ast_visit_stmt_specially_for_if_condition(bin_op->getRHS());
...
std::string ChipmunkAnotherDominoGenerator::ast_visit_transform(
  ...
  for (const auto *decl : dyn_cast<DeclContext>(tu_decl)->decls()) {
    if (isa<FunctionDecl>(decl) and
      ...
      res += "void " + dyn_cast<FunctionDecl>(decl)->getNameAsString() + "(" +
...
std::string
ChipmunkAnotherDominoGenerator::ast_visit_if_stmt(const IfStmt *if_stmt) {
  assert_exception(if_stmt);
  std::string ret;
  if (if_stmt->getElse() != nullptr && rand == 2) {
    ret += "if (!(" +
           ast_visit_stmt_specially_for_if_condition(if_stmt->getCond()) +
           ")) {" + ast_visit_stmt(if_stmt->getElse()) + " ;}";
    ret += "else if (" +
           ast_visit_stmt_specially_for_if_condition(if_stmt->getCond()) +
    ...
    if (c_to_sk.size() != 0) {
      ...
      for (std::map<std::string, std::string>::const_iterator it =
               c_to_sk.begin();
           it != c_to_sk.end(); it++) {
        useless_if_condition +=
            " && (" + it->first + "==" + it->first + "+ 1 - 1)";
      ...
      ret += "if (" +
             ast_visit_stmt_specially_for_if_condition(if_stmt->getCond()) +
             useless_if_condition + ") {" + ast_visit_stmt(if_stmt->getThen()) +
             ";" + useless_if_statement + "}\n";
...
std::string
ChipmunkAnotherDominoGenerator::ast_visit_bin_op(const BinaryOperator *bin_op) {
  assert_exception(bin_op);
  return ast_visit_stmt(bin_op->getLHS()) +
         std::string(bin_op->getOpcodeStr()) + ast_visit_stmt(bin_op->getRHS());
}

std::string ChipmunkAnotherDominoGenerator::ast_visit_decl_ref_expr(
  ...
  if (it == c_to_sk.end()) {
    ...
    if (s.find('.') != std::string::npos) {
      ...
      name =
          "tmp_" + std::to_string(round) + "_" + std::to_string(count_stateful);
      count_stateful++;
      c_to_sk[s] = name;
    }
  }
...
std::string ChipmunkAnotherDominoGenerator::ast_visit_member_expr(
...
std::string ChipmunkAnotherDominoGenerator::ast_visit_array_subscript_expr(
